...
# Uzimamo mail i odmah skidamo nevidljive razmake i pretvaramo u mala slova
admin_from_env = os.getenv("ADMIN_EMAILS", "").strip().lower()
secret_key_from_env = os.getenv("SECRET_KEY", "").strip()
algorithm_from_env = os.getenv("ALGORITHM", "HS256").strip()
access_token_expire_minutes_from_env = int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES", "30").strip())
verification_token_expire_hours_from_env = int(os.getenv("VERIFICATION_TOKEN_EXPIRE_HOURS", "24").strip())
...
# Funkcija za dohvatanje baze (koristi se kao dependency u ruterima)
def get_db():
...
def create_jwt_token(data: dict, expires_delta: timedelta = None):
    ...
    encoded_jwt = jwt.encode(to_encode, secret_key_from_env, algorithm=algorithm_from_env)
    return encoded_jwt
...
async def create_user(db: db_dependency, user: UserCreate):
    ...
    new_user = models.User(
        ...
        role=initial_role, # postavljamo rolu na osnovu emaila
        is_verified=False,
        is_active=True
    )
    ...
    expires_delta=timedelta(hours=verification_token_expire_hours_from_env))
...
###### end of registration and verification routes ######
...
async def update_user(db: db_dependency, user_id: int = Path(..., gt=0), user_update: UserCreate = None, current_user: models.User = Depends(get_current_user)):
    ...
    if user_update.city:
        user_to_update.city = user_update.city
    if user_update.country:
        user_to_update.country = user_update.country
    if user_update.zip_code:
        user_to_update.zip_code = user_update.zip_code
...
######## promjena emaila (samo vlasnik) - dvofazna verifikacija #######
...
async def forgot_password(email: ForgotPasswordSchema, db: Session = Depends(get_db)):
    ...
    # Use a relative reset link (avoid missing base URL). If you have a FRONTEND_URL or BASE_URL,
    # replace '/' with that variable, e.g. f"{BASE_URL}/users/reset-password/{token}"
    reset_link = f"/users/reset-password/{token}"
...
####### end of forgot/reset password routes #######
...
async def delete_user(db: db_dependency, user_id: int = Path(..., gt=0), current_user: models.User = Depends(get_current_user)):
    user = db.query(models.User).filter(models.User.id == user_id).first()
    if not user:
    ...
    is_owner = current_user.id == user.id #da li je vlasnik
